...
int main() {
    ...
    fault = mm.ref_page_usr(0, 10); // Page 10, PID 0
    print_test_result("Falta de pagina (Miss) na primeira referencia", fault == 1);
    
    fault = mm.ref_page_usr(0, 20); // Page 20, PID 0
    fault = mm.ref_page_usr(0, 30); // Page 30, PID 0
    
    fault = mm.ref_page_usr(0, 10); // Page 10 de novo
    ...
    mm.ref_page_usr(0, 40); // 4ª pagina. WS cheio: [20(mais antiga), 30, 10, 40(mais nova)]
    
    fault = mm.ref_page_usr(0, 50); // 5ª pagina. Deve ejetar a pagina 20 (LRU).
    print_test_result("Falta de pagina ao exceder Working Set", fault == 1);
    
    fault = mm.ref_page_usr(0, 20); // Como a 20 foi ejetada, pedir ela de novo DEVE dar page fault
    print_test_result("Pagina 20 foi ejetada corretamente (LRU funcionou)", fault == 1);

    fault = mm.ref_page_usr(0, 10); // A pagina 10 não foi ejetada, DEVE ser um Hit.
    ...
    for (int i = 1; i <= 4; i++) mm.ref_page_usr(1, i); // PID 1 usa 4 frames
    for (int i = 1; i <= 4; i++) mm.ref_page_usr(2, i); // PID 2 usa 4 frames

    print_test_result("Memoria acusa estar cheia (12/12 frames ocupados)", mm.is_usr_memory_full() == true);
    ...
    fault = mm.ref_page_usr(1, 5); // Ejeta a página 1 do PID 1.
    print_test_result("Substituicao feita com memoria global lotada sem crash", fault == 1);
    
    fault = mm.ref_page_usr(1, 1); // Confirma que a página 1 do PID 1 foi a ejetada
    ...
    mm.free_process_memory(2); // Mata o processo 2 (libera 4 frames)
    
    print_test_result("Memoria acusa ter espaco apos liberar processo", mm.is_usr_memory_full() == false);
    
    // Cria um novo processo PID 3, que deve conseguir usar os frames recém-liberados
    fault = mm.ref_page_usr(3, 100);
